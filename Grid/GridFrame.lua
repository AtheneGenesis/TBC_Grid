-- GridFrame.lua
-- /script Grid:Print(GridFrame.db.profile.NewColor.r)
--{{{ Libraries
...
function GridFrameClass.prototype:SetBarColor(r, g, b, a)
	if GridFrame.db.profile.UseCustomColors then
		local color1 = GridFrame.db.profile.CustomColor1
		local CustomColor1r = color1.r
		local CustomColor1g = color1.g
		local CustomColor1b = color1.b
		local CustomColor1a = color1.a
		local color2 = GridFrame.db.profile.CustomColor2
		local CustomColor2r = color2.r
		local CustomColor2g = color2.g
		local CustomColor2b = color2.b
		local CustomColor2a = color2.a
		if GridFrame.db.profile.invertBarColor then
			self.frame.Bar:SetStatusBarColor(CustomColor2r, CustomColor2g, CustomColor2b, 1)
			self.frame.BarBG:SetVertexColor(CustomColor1r, CustomColor1g, CustomColor1b, 1)
		else
			self.frame.Bar:SetStatusBarColor(CustomColor1r, CustomColor1g, CustomColor1b, 1)
			self.frame.BarBG:SetVertexColor(CustomColor2r, CustomColor2g, CustomColor2b, 1)
		end
	else
		if GridFrame.db.profile.invertBarColor then
...
function GridFrameClass.prototype:UpdateHealingBarColor()
	...
	if GridFrame.db.profile.invertBarColor and not GridFrame.db.profile.UseCustomColors then
	...
	elseif self.frame.HealingBar:GetValue() > 0 then
	    ...
	    if GridFrame.db.profile.UseCustomColors then
			...
			local CustomColor1b = color1.b
			local CustomColor1a = color1.a
			self.frame.HealingBar:SetStatusBarColor(r, g, b, healingBar_alpha)
		else
...
local function getcoloroptionshidden()
	return not GridFrame.db.profile.UseCustomColors
end
...
GridFrame.defaultDB = {
	...
	CustomColor1 = { r = 0.125, g = 0.125, b = 0.125, a = 1 },
    CustomColor2 = { r = 0.309, g = 0.309, b = 0.309, a = 1 },
	showTooltip = "OOC",
	textlength = 4,
...
GridFrame.options = {
	...
	args = {
		...
		["customcolors"] = {
			type = "toggle",
			name = L["Use custom bar colors"],
			desc = L["Desc custom bar colors"],
			order = 13,
			get = function ()
				return GridFrame.db.profile.UseCustomColors
			end,
			set = function (v)
				GridFrame.db.profile.UseCustomColors = v
				GridFrame:UpdateAllFrames()
			end,
		},
		["color1"] = {
			...
			desc = L["Desc Custom Color 1"],
			order = 14,
			hasAlpha = false,
			get = function ()
					local color = GridFrame.db.profile.CustomColor1
			...
			hidden = getcoloroptionshidden,
		},
		["color2"] = {
			type = "color",
			name = L["Custom Color 2"],
			desc = L["Desc Custom Color 2"],
			order = 15,
			hasAlpha = false,
			get = function ()
				local color = GridFrame.db.profile.CustomColor2
				return color.r, color.g, color.b, color.a
			end,
			set = function (r, g, b, a)
				local color = GridFrame.db.profile.CustomColor2
